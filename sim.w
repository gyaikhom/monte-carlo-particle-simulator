...
@** Simulation. The particle simulations are carried out in
batches. After a simulation has begun, it will only exit under two
conditions:
...
@ Function |simulate_particle(p,j,s,k)| simulates the particle in
|p[j]| by applying the physics processes. The resulting particle is
stored in |s[k]|. All of the secondary particles generated by the
physics processes are then stored in the subsequent array elements
following |s[k]|. This function returns the next valid array element
in |s|, which will be used in the next call to
|simulate_particle(p,j,s,k)|.
...
uint32_t simulate_particle(Particle *p, uint32_t j, Particle *s, uint32_t k)
{
	...
	@<Update secondary particles array@>;
	return k;
}
...
int simulate_batch(Batch *b)
{
	...
	while(i < b->n) {
		...
		*/
		k = simulate_particle(b->b[i].p, j, b->b[i].s, k);
		++j;
	    }
	    b->b[i].ns = k;
	    b->b[i].np = 0;
